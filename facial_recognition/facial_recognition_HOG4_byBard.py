...
def get_output_filename(image_file, output_folder, ext, filename_option, face_count):
    base_filename, file_extension = os.path.splitext(os.path.basename(image_file))
    if filename_option == "連番":
        face_number_str = str(face_count).zfill(3)
        return os.path.join(output_folder, f'{face_number_str}{file_extension}')
    elif filename_option == "元のファイル名と同じ":
        return os.path.join(output_folder, f'{base_filename}_{face_count}{file_extension}')
    else:  # "元のファイル名_crop" の場合
        return os.path.join(output_folder, f'{base_filename}_{face_count}_crop{file_extension}')
