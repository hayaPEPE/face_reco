...
def add_padding(image, size, color=(0, 0, 0)):
    h, w = image.shape[:2]
    if h == w:
...
def detect_faces(input_folder, output_folder, progress, size=512, margin=0.3, filename_option="連番", output_mode="カラー"):
    ...
    for i, image_file in enumerate(image_files):
        ...
        for face in faces:
            ...
            face_img_padded = add_padding(face_img, size)

            if output_mode == "モノクロ":
                face_img_gray = cv2.cvtColor(face_img_padded, cv2.COLOR_BGR2GRAY)
                cv2.imwrite(output_file, face_img_gray, [cv2.IMWRITE_TIFF_COMPRESSION, 1])
            else:
                cv2.imwrite(output_file, face_img_padded, [cv2.IMWRITE_TIFF_COMPRESSION, 1])
                face_count += 1
                face_count_local += 1
...
def main():
    ...
    start_button = tk.Button(root, text="開始", command=lambda: detect_faces(input_entry.get(), output_entry.get(), progress, int(size_entry.get()), float(margin_entry.get()), filename_option_combobox.get(), output_mode_combobox.get()))
